...
var last_selected_item: PlaceableItem
...
func _click_left_released(event: InputEvent, mouse_pos: Vector2):
	_click_released(event, mouse_pos)


func _click_right_released(event: InputEvent, mouse_pos: Vector2):
	_click_released(event, mouse_pos)


func _click(world_pos: Vector2) -> void:
	editor.get_hovered_objects()
	if not editor.selected_objects.empty() and editor.hovered_objects.empty():
		emit_signal("objects_selected", [])
	
	elif not editor.hovered_objects.empty():
		...
		if !Input.is_action_pressed("shift_modifier"): 
			hovered_object = closest_object
		else: 
			last_selected_item = editor.selected_item
...
func _click_released(event, mouse_pos):
	if hovered_object:
		if is_dragging:
			if is_copied: 
				hovered_object.set_property("position", hovered_object.position, true)
				editor.selected_item = last_selected_item
			else: 
				change_property(hovered_object, "position", hovered_object.position, old_pos)
			is_copied = false
			is_dragging = false
