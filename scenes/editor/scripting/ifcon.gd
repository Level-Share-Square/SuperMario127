...
func _ready():
 ...
 #check if any extra variables are created and add them to the options. "i" is not numerical; it is dictionary words (var names).
 for i in scripting.variables.keys():
	 #add item (i is the name)
	 variables.add_item(str(i))
