...
func load_settings():
	bg_index = area.header.sky
	fg_index = area.header.background
	...
	current_palette = area.header.background_palette
	...
	autoscroll_pick.value = area.header.bg_autoscroll_speed
...
#		CurrentLevelData.level_info.thumbnail_background_palette = current_palette
		area.header.background_palette = current_palette
...
#	CurrentLevelData.level_info.thumbnail_background = index
	area.header.background = index
	area.header.background_palette = 0
	fg_index = index
	_init_palette_dropdown()
	emit_signal("update_background")
...
func _on_autoscroll_change(value):
	area.header.bg_autoscroll_speed = value
	emit_signal("update_background")
