...
onready var shards = $CanvasLayer/HealthUI/Shards
...
func _process(_delta):
	...
	if shown and !last_shown:
		tween.interpolate_property(ui, "position",
			ui.position, Vector2(ui.position.x, 20), 0.50,
			Tween.TRANS_BACK, Tween.EASE_OUT)
		tween.start()
	elif !shown and last_shown:
		tween.interpolate_property(ui, "position",
			ui.position, Vector2(ui.position.x, -80), 0.50,
