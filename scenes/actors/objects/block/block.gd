...
func _on_hit_body_entered(body):
	...
	if body.name.begins_with("Character"):
		if hit_direction.y <= 0 and (hit_direction.x <= 0.650 and hit_direction.x >= -0.650): #0.650 is a little less than the approximate value of a diagonal normal
			block_hit(Vector2.UP)
		elif body.state != null && body.state.name == "GroundPoundEndState" and hit_direction.y >= 0 and (hit_direction.x <= 0.650 and hit_direction.x >= -0.650):
			block_hit(Vector2.DOWN)

	elif body.name.begins_with("Shell"):
		if (hit_direction.y <= 0.650 and hit_direction.y >= -0.650): #0.650 is a little less than the approximate value of a diagonal normal
