...
onready var editor_collision = $EditorHitbox/CollisionShape2D
...
func _ready():
	...
	if scale != Vector2.ONE: # Nothing to do on default scale
		...
		editor_collision.shape = collision_shape.shape
...
func update_property(key: String, value):
	if key == "size":
		...
		collision_shape.shape = collision_shape.shape.duplicate()
		# Modify the extents by the scale to get the desired collision shape
		collision_shape.shape.extents = Vector2(value.x * 0.5 * scale.x,\
												value.y * 0.5 * scale.y)
		editor_collision.shape = collision_shape.shape
