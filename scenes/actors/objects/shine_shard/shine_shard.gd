...
func collect(body):
	if enabled and !collected and body.name.begins_with("Character") and !body.dead:
		CurrentLevelData.level_data.vars.shine_shards_collected[CurrentLevelData.area][0] += 1
		CurrentLevelData.level_data.vars.shine_shards_collected[CurrentLevelData.area][1].append(id)
		var player_id = 1
