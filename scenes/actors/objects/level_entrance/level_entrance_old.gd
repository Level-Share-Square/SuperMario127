...
func _ready():
	if mode == 0:
		...
		if is_enabled_and_on_ground() and (CurrentLevelData.checkpoint_data.current_checkpoint_id == -1 or CurrentLevelData.vars.transition_data != []):
			...
			transition_character_data = CurrentLevelData.vars.transition_character_data
			if character_string != "character":
				transition_character_data = CurrentLevelData.vars.transition_character_data_2
			...
			if transition_data.size() == 0:
				...
				for tp_obj in CurrentLevelData.vars.teleporters:
