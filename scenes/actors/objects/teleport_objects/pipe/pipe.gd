...
var pipe_mode = TELEPORT_MODE.LOCAL
var area_id := 0
var pipe_tag := "default_pipe"
var pipe_destination_tags : PoolStringArray = []
var color := Color(0, 1, 0)


func _set_properties():
	savable_properties = ["area_id", "pipe_tag", "color", "pipe_destination_tags", "pipe_mode"]
	editable_properties = ["area_id", "pipe_tag", "color", "pipe_destination_tags", "pipe_mode"]
	
func _set_property_values():
	set_property("area_id", area_id, true)
	set_property("pipe_tag", pipe_tag, true)
	set_property("color", color, true)
	set_property("pipe_mode", pipe_mode, true)
	set_property("pipe_destination_tags", pipe_destination_tags, true)
...
func _process(delta): #WHY IS THIS IN _PROCESS
