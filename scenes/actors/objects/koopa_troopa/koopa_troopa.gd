...
func self_queue_free():
	queue_free()
	is_self_queued_free = true

var rainbow_animation_speed := 500
...
func _physics_process(delta):
	...
	if mode != 1 and enabled and !dead and loaded:
				...
				if !is_instance_valid(shell):
					...
					if !winged:
						...
						if !right_check.is_colliding() or body.global_position.x > (level_bounds.end.x * 32 - 1) or body.test_move(body.global_transform, Vector2(0.1, 0)):
							facing_direction = -1
		elif !is_instance_valid(shell): # This shouldn't happen, but just in case
			hit = false
		else:
		...
		if is_instance_valid(shell) and !is_body_queued_free:
			body.queue_free() # Moved here to possibly prevent crashes
			is_body_queued_free = true
