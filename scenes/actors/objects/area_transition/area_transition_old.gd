...
func _ready():
	...
	CurrentLevelData.vars.teleporters.append([destination_tag.to_lower(), self])
	if mode == 1:
	...
	if !teleportation_mode:
		...
		if pair.object_type == "area_transition":
			...
			if character.player_id == 0:
				...
				CurrentLevelData.vars.transition_character_data_2.append(AreaTransitionHelper.new(character.velocity, character.state, character.facing_direction, to_local(character.position), self.vertical))
...
func pipe_exit_anim_finished(character : Character):
	...
	if (CurrentLevelData.vars.transition_character_data.size() >= 7
	|| CurrentLevelData.vars.transition_character_data_2.size() >= 7):
...
func get_character_transition_data(character : Character) -> Array:
		if character.player_id == 0:
			return CurrentLevelData.vars.transition_character_data
		else:
			return CurrentLevelData.vars.transition_character_data_2
