...
func pick_item_recursive_find(id: int, group: Node) -> PlaceableItemOld:
	if group is PlaceableItemOld:
		return group as PlaceableItemOld if group.is_object and group.object_id == id else null
	else:
...
func pick_tile_recursive_find(tileset_id: int, tile_id: int, group: Node) -> PlaceableItemOld:
	if group is PlaceableItemOld:
		return group as PlaceableItemOld if\
