...
func _input(event):
	...
	var character2_node = player.get_node(player.character2)
	if !scene_transitions.transitioning and !mode_switcher.get_node("ModeSwitcherButton").switching_disabled and event.is_action_pressed("30_fps") and !(get_tree().paused and !PhotoMode.enabled):
