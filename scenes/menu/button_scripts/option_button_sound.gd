...
func _ready() -> void:
	...
	connect("item_selected", self, "on_item_pressed")
	#warning-ignore:return_value_discarded
	connect("item_focused", self, "on_item_focus_entered")
	#warning-ignore:return_value_discarded
...
func on_item_focus_entered(_index: int = -1) -> void:
	if disabled: return
	hover_sound.play()


func on_item_pressed(_index: int = -1) -> void:
	click_sound.play()
