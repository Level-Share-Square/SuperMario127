...
func transition_to_directory(working_folder: String, is_campaign: bool):
	...
	load_directory(working_folder, is_campaign)
...
func load_directory(working_folder: String, is_campaign: bool):
	...
	list_handler.is_campaign = is_campaign
	...
	for campaign in sort.get(sort_file_util.CAMPAIGNS, []):
		add_campaign_card(campaign, working_folder, not is_dev_folder)
...
func add_campaign_card(
	...
	var level_grid: GridContainer = list_handler.level_grid
	var card_node: CampaignCard = campaign_card_scene.instance()
