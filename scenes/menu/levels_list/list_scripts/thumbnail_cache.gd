...
func queue_thread():
	...
	for thumbnail in thumbnail_queue:
		...
		if not saved_levels_util.file_exists(thumbnail_path):
			...
			thumbnail_path = saved_levels_util.get_level_thumbnail_path(level_id, working_folder)
		
		var image: ImageTexture = saved_levels_util.get_image_from_path(thumbnail_path)
